...
export class GiveawayDeleteAutocompleteHandler extends InteractionHandler {
	...
	public override async parse(interaction: AutocompleteInteraction) {
		if (interaction.commandName !== 'delete') return this.none();
		...
		const all = [...active, ...completed].sort((a, b) => b.startTime.getTime() - a.startTime.getTime());
		const choices = all
			.map((g) => ({
