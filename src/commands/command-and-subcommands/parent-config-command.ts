...
export class ParentCommandExample extends Subcommand {
	...
	public override registerApplicationCommands(registry: ApplicationCommandRegistry) {
		registry.registerChatInputCommand((ctx) => {
			...
			ctx.addSubcommandGroup((sc) => sc.setName('managers').setDescription("Configure Bot's managers settings")); // Each group of subcommands must have a unique name and description and be registered like this.
			...
			return ctx.setName(this.name).setDescription('Example parent command');
		});
	}
}
